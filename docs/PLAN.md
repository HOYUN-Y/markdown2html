...
배포본은 주소만 알면 누구나 쓸 수 있는 상태였다. 게다가 세 도구의 작업 내용을 **DB에
저장**하기로 하면서(사용자 결정) 이야기가 달라졌다 — 지금까지는 저장이 브라우저
localStorage뿐이라 남에게 보일 것이 없었지만, 서버에 남기 시작하면 가려야 한다.

### 확인한 것 — Vercel 플랫폼 보호는 못 쓴다

README에 "가리려면 Deployment Protection을 켠다"고 적어 뒀는데 **틀렸다.**
Hobby 플랜에서는 프리뷰만 가려지고 **프로덕션 도메인은 계속 공개**다. 프로덕션까지
가리려면 Pro 이상, 비밀번호 방식은 Enterprise 또는 Pro + 월 $150 애드온이다.
→ 앱에서 직접 막는 수밖에 없다. README의 그 문장도 고쳤다.

### 1단계 — 로그인

- [x] `auth.py` — 비밀번호 하나 + 서명된 쿠키 세션. 서버에 세션 저장소를 두지 않는다
      (Vercel 함수는 요청마다 다른 인스턴스일 수 있다)
- [x] `before_request`로 **모든 요청 앞에** 세운다. 화면마다 데코레이터를 붙이면
      도구를 새로 만들 때 빠뜨린 화면만 공개된다 — 기본을 '막힘'으로 둔다
- [x] `/login`·`/logout`, 로그인 화면, 툴바의 로그아웃 버튼
- [x] 설정이 없을 때: **로컬은 열고 배포본은 503으로 잠근다.** 설정을 빠뜨린 채
      올라가면 사이트가 통째로 공개되므로 닫는 쪽으로 실패한다
- [x] `SECRET_KEY`가 없으면 배포본은 뜨지 않는다 (임시 키를 쓰면 인스턴스마다 달라
      로그인이 계속 풀린다)
- [x] 쿠키 조이기 — HttpOnly · SameSite=Lax · Secure(배포본에서만. 로컬은 http라
      켜면 쿠키가 아예 안 실린다)
- [x] `next` 되돌아가기는 **우리 경로만** 따른다 (열린 리다이렉트는 피싱 발판이 된다)
- [x] 해시는 `pbkdf2:sha256`으로 못 박음 — Werkzeug 기본값(scrypt)은 OpenSSL 빌드에
      따라 없을 수 있고(로컬 파이썬에 없다) 그러면 환경이 갈리는 순간 검증이 실패한다
- [x] `scripts/make_password_hash.py` — 비밀번호를 인자로 받지 않는다(셸 기록·ps 노출)
- [x] 테스트 12개 (총 135개). 개발자가 로컬에 해시를 설정해도 나머지 테스트가
      깨지지 않게 `test_pages.py`가 도는 동안 로그인을 꺼 둔다
- [ ] **환경변수 등록** — 사용자가 Vercel 대시보드에서 직접 (해시·SECRET_KEY)

### 2단계 — DB 저장

Neon Postgres, 도구별 문서 여러 개(제목·목록·불러오기·삭제).
새 의존성 `psycopg[binary]` 하나. Vercel 함수는 디스크가 없어 SQLite를 못 쓴다.

저장하는 것은 **원본뿐이다** — 마크다운 원문·mermaid 소스·`.drawio` XML.
변환 결과(HTML·SVG·PNG)는 넣지 않는다. 다시 만들 수 있고, 넣어 두면 원본을 고쳤을 때
조용히 어긋난다. (사용자 확인: "코드 형태로 저장하면 불러와지는 거니까")

- [x] `db.py` — `documents` 테이블 하나. 도구가 셋이어도 저장 모양이 같아 나누지 않는다
- [x] 테이블은 처음 저장할 때 만들어진다(`IF NOT EXISTS`) — 마이그레이션 도구를 안 들인다
- [x] `GET/POST /api/docs`, `GET/DELETE /api/docs/<id>` — 로그인은 문지기가 이미 막는다
- [x] `public/js/docs.js` — 저장·불러오기 막대. 세 도구가 같은 조각을 쓴다
- [x] 자동 저장은 계속 localStorage가 맡는다. 서버는 **누를 때만** —
      draw.io는 도형을 끌 때마다 신호가 와서 매번 보내면 요청이 폭주한다
- [x] DB가 없으면 **막대를 아예 안 그린다.** 도구 자체는 그대로 다 된다
- [x] 삭제는 한 번 더 누르게 한다(대화상자로 흐름을 끊지 않고, 실수로 지워지지도 않게)
- [x] 테스트 14개 (총 149개). **진짜 DB를 붙이지 않는다** — 붙이면 네트워크와 남의
      서비스에 매여, 연결이 안 되는 날 코드와 상관없이 빨개진다
- [ ] **Neon 연결** — 사용자가 Vercel 대시보드에서 (Storage → Neon)
- [ ] 배포본에서 저장·불러오기·삭제를 사람이 눌러 확인 (실제 SQL은 여기서만 검증된다)
