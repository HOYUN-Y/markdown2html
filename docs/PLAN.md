...
Neon Postgres(Vercel 마켓플레이스), 도구별 문서 여러 개(제목·목록·불러오기·삭제).
새 의존성 `psycopg[binary]` 하나. Vercel 함수는 디스크가 없어 SQLite를 못 쓴다.

- [ ] Neon 연결 (사용자가 Vercel 대시보드에서)
- [ ] `documents` 테이블 — `id · tool · title · content · updated_at`
- [ ] `POST/GET/DELETE /api/docs` — 로그인 뒤에 있으므로 문지기는 이미 서 있다
- [ ] 세 도구 화면에 저장·불러오기·목록 UI
- [ ] DB가 없을 때(로컬) **화면이 죽지 않고** 저장 기능만 꺼지게
