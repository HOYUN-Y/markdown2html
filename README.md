...
세 도구 모두 **제목을 붙여 서버에 저장**하고 목록에서 다시 불러온다. 기기가 바뀌어도
남는다 — `localStorage`는 브라우저마다 따로라 노트북에서 그린 그림이 데스크톱에 없다.

역할을 나눠 두었다.

| | 무엇 | 언제 |
|---|---|---|
| `localStorage` | **작업 중인 임시본** | 자동으로, 계속 덮어쓴다 |
| 서버(Postgres) | **남겨 둘 것** | `저장`을 눌렀을 때만 |

draw.io는 도형을 끌 때마다 자동 저장 신호가 오는데, 그걸 매번 서버로 보내면 요청이
폭주한다. 그래서 서버 저장은 사람이 누를 때만 움직인다.

**저장하는 것은 원본뿐이다.** 마크다운 원문 · mermaid 소스 · `.drawio` XML만 넣고
변환 결과(HTML·SVG·PNG)는 넣지 않는다 — 언제든 다시 만들 수 있고, 넣어 두면 원본을
고쳤을 때 조용히 어긋난다.

```
documents
  id · tool · title · content · created_at · updated_at
```

도구가 셋인데 테이블은 하나다. 저장하는 모양이 '제목 붙은 원본 한 덩어리'로 같아서,
나눌 이유가 없다. 소유자 컬럼도 없다 — 로그인 뒤에 있고 쓰는 사람이 한 명이다.

### 붙이는 법

Vercel 프로젝트 → **Storage → Neon(Postgres) 연결**. 연결하면 접속 문자열이 환경변수로
자동 주입된다(`DATABASE_URL` 계열 — `db.py`의 `URL_ENV_NAMES`가 순서대로 찾는다).
**테이블은 처음 저장할 때 알아서 만들어진다**(`CREATE TABLE IF NOT EXISTS`) —
테이블 하나짜리라 마이그레이션 도구를 들이지 않았다.

SQLite를 쓰지 않는 이유는 Vercel 함수에 **디스크가 없기** 때문이다. 요청마다 다른
인스턴스일 수 있고 파일시스템도 사라진다.

### 붙이지 않으면

저장 막대가 **화면에 아예 안 나온다**(눌러도 아무 일이 없는 버튼을 두지 않는다).
도구 자체는 그대로 다 된다. API를 직접 부르면 503과 이유를 돌려준다.
...
| `DATABASE_URL` | Neon을 연결하면 **자동으로 들어온다.** 직접 만들 필요 없다 |
...
계획을 문서 밖에 두지 않으려는 것이다. 지금은 셋 다 올라와 있다.
...
상단 `Mermaid 다이어그램` 탭(`/mermaid`). 왼쪽에 소스를 쓰면 오른쪽에 바로 그려지고,
템플릿 다섯(플로우차트·시퀀스·클래스·간트·ER)에서 시작할 수 있다.

| 가져가는 방법 | 쓸 곳 | 대가 |
|---|---|---|
| **코드블록 복사** | 블로그 마크다운 원문에 붙여넣는다. 변환 탭이 알아서 처리한다 | Blogger에 **테마 스니펫이 있어야** 그려진다. 나중에 고칠 수 있다 |
| **SVG 저장** | 스니펫 없이도 보인다. 확대해도 안 깨진다 | 고치려면 원본 소스를 따로 보관해야 한다 |
| **PNG 저장** | 어디서나 확실히 보인다 | 확대하면 흐리다. 글자 검색도 안 된다 |

**이 화면은 서버를 부르지 않는다.** mermaid는 브라우저에서 그리는 라이브러리이고,
그렇게 그려야 Blogger에 붙여넣었을 때와 같은 그림이 나온다 — Blogger 쪽도 테마 스니펫의
같은 mermaid가 같은 소스를 그린다. 버전·테마는 `converter/snippet.py`가 유일한 출처다.

알아둘 것:

- **PNG는 미리보기와 다른 설정으로 다시 그린다.** mermaid 기본값은 라벨을
  `<foreignObject>`(HTML)로 그리는데, 그 SVG를 canvas에 올리면 브라우저가 그 부분을
  래스터화하지 못해 **글자가 통째로 빠진다.** PNG 경로만 `htmlLabels:false`로 재렌더하므로
  글자 모양이 미리보기와 미세하게 다를 수 있다.
- **미리보기 배경 칩은 배경만 바꾼다.** 다이어그램 테마는 `neutral` 고정이라 글자가 어둡다.
  검은 배경 Blogger 테마에서 읽히는지 확인하는 용도다.
- 문법이 틀린 동안에도 **마지막으로 성공한 그림을 지우지 않는다.** 흐리게 두고 오류만 알린다.
- 템플릿을 누르면 원고가 통째로 바뀌지만 배너의 **되돌리기**로 직전 원고가 돌아온다.

## draw.io로 그리기

상단 `draw.io 다이어그램` 탭(`/drawio`). 편집기는 **draw.io 공식 임베드**를 그대로 쓴다 —
도형을 끌어다 놓으면 선이 알아서 달라붙고, 도형 라이브러리·단축키·되돌리기가 전부 있다.
우리가 만든 것은 바깥 틀과 내보내기뿐이다.

| 가져가는 방법 | 쓸 곳 | 대가 |
|---|---|---|
| **인라인 조각 복사** | Blogger `HTML 보기`에 바로 붙여넣기. 이미지를 어디에도 안 올린다 | 본문이 길어진다. 편집기가 태그를 손대면 깨질 수 있다 |
| **SVG 저장** | 이미지로 올린다. 확대해도 안 깨진다 | — 원본 XML이 파일 안에 있어 **다시 불러와 편집된다** |
| **PNG 저장** | 어디서나 확실히 보인다 | 확대하면 흐리다 |
| **`.drawio` 저장** | 원본. draw.io 데스크톱·VS Code 확장과도 호환 | 그림이 아니라 원본이라 글에 바로 못 넣는다 |

**이 화면도 서버를 부르지 않는다.** 그림은 브라우저와 편집기 iframe 안에서만 오간다 —
우리 서버로 오지 않고, 저장되는 곳은 브라우저(localStorage)와 사용자가 내려받은 파일뿐이다.
다만 편집기 자체는 **제3자(`embed.diagrams.net`)의 앱**이다. Apache 2.0에 계정도 필요
없지만, 남의 코드를 우리 화면에 얹고 있다는 사실은 알고 쓰는 편이 낫다.

알아둘 것:

- **인라인 조각은 id에 접두사를 붙인다.** draw.io SVG는 화살표 머리·그라디언트를
  `<defs>`의 id로 참조해서, 한 글에 그림을 둘 넣으면 **두 번째가 첫 번째의 정의를 먹어
  화살표가 사라진다.** 오류가 안 나서 눈으로만 보이는 고장이라 미리 막아 뒀다.
- 조각은 `<div>`로 감싸 내보낸다. 마크다운 원문에 그냥 `<svg>`를 넣으면 그림 **안쪽
  글자에까지** 마크다운 문법이 먹어 `<text>*별*</text>`가 `<em>`이 된다.
- 편집기 테마는 `dark=0`으로 고정했다. 다크로 뜨면 내보낸 그림의 글자가 흰색이 되어
  흰 배경 블로그에서 **글자만 안 보인다.**
- 자동 저장이 브라우저 한도를 넘으면 **알리고 멈춘다.** 그림 안에 사진이 들어가면
  원본이 MB 단위가 된다 — 그때는 `.drawio`로 저장해 두는 편이 안전하다.
...
db.py               저장한 문서 (Postgres). converter/는 이걸 import하지 않는다
...
public/             정적 파일 (Vercel이 CDN에서 직접 내보낸다)
  ...
  js/docs.js          저장·불러오기 막대 — 세 도구가 함께 쓴다
scripts/            개발용 (비밀번호 해시 만들기 · 배포 번들에서 제외)
tests/              unittest 149개 (정방향 55 · 역방향 49 · 화면 19 · 로그인 12 · 저장 14)
...
- 목록 뒤 목록이 앞 목록에 흡수되던 문제는 **`sane_lists` 확장으로 고쳤다**(v0.6.1).
  블로그도 같은 확장을 쓰므로 출력이 갈라지지 않는다. 부작용으로 `3.`처럼 1이 아닌
  번호로 시작하면 `<ol start="3">`이 된다 — 의도에 가깝다.
- **인라인 SVG 조각을 Blogger에 붙여넣었을 때 살아남는지는 아직 확인하지 못했다**
  (계정 접근이 필요하다). 편집기가 HTML을 다시 직렬화하면서 `viewBox`·`foreignObject`의
  대소문자를 못 살릴 가능성이 있다. 확실히 하려면 SVG·PNG 파일로 저장해 이미지로 올린다.
- 우리 HTML 파서가 태그·속성 이름을 소문자로 누른다 — `viewBox` → `viewbox`.
  브라우저가 되살리므로 화면에는 문제가 없지만, 파일로 저장하는 경로에서는 드러날 수 있다.
