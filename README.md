...
- **Blogger 테마 배경** — 아래 [팔레트](#팔레트--blogger-테마-밝기에-맞추기) 참고. **이걸 먼저 맞춰야 한다.**
...
`prefers-color-scheme`으로 자동 대응이 안 된다. 그래서 셋 중에 고른다.

| 팔레트 | 언제 | 대가 |
|---|---|---|
| `light` (기본) | 흰색·밝은 배경 테마 | 검은 테마에 쓰면 **본문이 배경에 묻힌다** |
| `dark` | 검은·어두운 배경 테마 | 나중에 테마를 밝게 바꾸면 기존 글이 전부 안 읽힌다 |
| `inherit` | 테마를 바꿀 수도 있을 때 | 글자색을 Blogger 테마에 맡긴다 — 어떤 테마에도 맞지만 블로그와 **똑같은 색은 아니다** (링크가 파란색 대신 본문색+밑줄) |

`light`/`dark`는 블로그의 라이트·다크 모드 토큰을 **그대로** 쓴다. 코드블록은 팔레트와
무관하게 늘 어둡다 — 블로그도 두 테마 모두 어둡게 둔다(`blog.css:435`).
