...
계획을 문서 밖에 두지 않으려는 것이다. 지금 그렇게 올라와 있는 것: draw.io 임베드.
...
diagrams.py         다이어그램 시작 템플릿 5종 (편집기 화면 전용 · converter/ 밖)
cli.py              파일 단위 변환
templates/          화면 — 블로그 전용 페이지의 디자인 토큰을 따름
public/             정적 파일 (Vercel이 CDN에서 직접 내보낸다)
  js/diagram.js       다이어그램 편집기 동작 (렌더·내보내기 · 서버를 부르지 않는다)
tests/              unittest 109개 (정방향 55 · 역방향 44 · 화면 10)
